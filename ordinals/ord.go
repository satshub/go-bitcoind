...
type InscriptionRequest struct {
	...
	EnableRBF      bool   //enable: nSequence number less than (0xffffffff - 1); defined in bip125
}
...
type blockchainClient struct {
	...
	btcApiClient memPool.BTCAPIClient
}
...
func (tool *InscriptionTool) _initTool(net *chaincfg.Params, request *InscriptionRequest) error {
	...
	totalRevealPrevOutput, err := tool.buildEmptyRevealTx(request.SingleRevealTxOnly, destinations, revealOutValue, request.FeeRate, request.EnableRBF)
	if err != nil {
		return err
	}
...
func (tool *InscriptionTool) buildEmptyRevealTx(singleRevealTxOnly bool, destination []string, revealOutValue, feeRate int64, enableRBF bool) (int64, error) {
...
func (tool *InscriptionTool) buildLockedScript(address string) *[]byte {
...
func (tool *InscriptionTool) buildCommitTx(commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64, changeAddress string, enableRBF bool) error {
...
func (tool *InscriptionTool) writeLog() {
	// 打开文件，如果文件不存在则创建，以追加写入的方式
	fileName := "failure-txs.log"
	file, err := os.OpenFile(fileName, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0644)
	if err != nil {
		fmt.Println("open failure-txs.log err:", err)
		return
	}
	defer file.Close()
	//txHex := hex.EncodeToString(buf.Bytes())

	_, err = file.WriteString("\ncommit txid:" + tool.commitTx.TxHash().String() + "\n")
	if err != nil {
		println("write commitTx to log file err:%s", err.Error())
		return
	}

	//commit tx 记录
	buf := bytes.NewBuffer(make([]byte, 0, tool.commitTx.SerializeSize()))
	if err := tool.commitTx.Serialize(buf); err != nil {
		println("when write to log file, serialize reveal tx err:", err.Error())
		return
	}
	file.WriteString(hex.EncodeToString(buf.Bytes()) + "\n")

	//写入所有reveal txs
	for i := range tool.revealTx {
		//写入revealTx提示信息
		msg := fmt.Sprintf("reveal txid: %s - %d - %s\n", tool.revealTx[i].TxHash().String(), i, tool.commitTx.TxHash().String())
		if _, err := file.WriteString(msg); err != nil {
		}

		// 写入单条reveal tx
		buf := bytes.NewBuffer(make([]byte, 0, tool.revealTx[i].SerializeSize()))
		if err := tool.revealTx[i].Serialize(buf); err != nil {
			println("when send raw tx, serialize reveal tx err:", err.Error())
			return
		}

		if _, err := file.WriteString(hex.EncodeToString(buf.Bytes()) + "\n"); err != nil {
			println("write reveal tx to log file err:", err.Error())
			return
		}
...
func (tool *InscriptionTool) Inscribe() (commitTxHash *chainhash.Hash, revealTxHashList []*chainhash.Hash, inscriptions []string, failTxIndex []int, fees int64, err error) {
	...
	if err != nil {
		tool.writeLog()
		return nil, nil, nil, failTxIndex, fees, errors.Wrap(err, "send commit tx error")
	}
	...
	for i := range tool.revealTx {
		for j := 0; j < 8; j++ {
			...
			if err != nil && j == 7 {
				...
				txHex := hex.EncodeToString(buf.Bytes())
				failTxIndex = append(failTxIndex, i)
				println("send reveal tx error, index:%d,txid:%s, rawtx:%s", i, tool.revealTx[i].TxHash().String(), txHex)
				continue
			}

			if err != nil {
				time.Sleep(time.Second * time.Duration(math.Pow(2, float64(j))))
				continue
			}
	...
	return commitTxHash, revealTxHashList, inscriptions, failTxIndex, fees, nil
}
...
func (tool *InscriptionTool) GetCommitAddress(index int) (string, error) {
	pkSript := tool.txCtxDataList[index].commitTxAddressPkScript
...
func (tool *InscriptionTool) TransmitAddress(index int) string {
	return tool.txCtxDataList[index].commitTxAddress
}
