...
func BuildInscriptionTxs(privateKey *btcec.PrivateKey, utxo []*Utxo, mime string, content []byte, feeRate int64, revealValue int64, net *chaincfg.Params, inscriptionAddData []byte, opReturnData []byte) ([]byte, []byte, int64, error) {
	...
	receiver, err := GetUtilP2TRAddress(pubKey, net)
	if err != nil {
	...
	// 2. build reveal tx
	revealTx, totalPrevOutput, revealFee, err := buildEmptyRevealTx(receiver, inscriptionScript, revealValue, feeRate, opReturnData)
	if err != nil {
		return nil, nil, 0, err
	}
	// 3. build commit tx
	...
	commitTx, commitFee, err := buildCommitTx(utxo, out, feeRate, nil, true)
	if err != nil {
		return nil, nil, 0, err
	}
	// 4. completeRevealTx
	...
	return commitTxBytes, revealTxBytes, revealFee + commitFee, nil
}
...
func BuildRuneEtchingDummyRevealTx(privateKey *btcec.PrivateKey, runeOpReturnData []byte, runeCommitment []byte,
	...
	//inscriptionPkScript, _ := txscript.PayToAddrScript(inscriptionAddress)
	// 2. build reveal tx
	_, _, revealFee, err := buildEmptyRevealTx(receiver, inscriptionScript, revealValue, feeRate, runeOpReturnData)
	if err != nil {
		return 0, "", err
	}
	return revealFee, inscriptionAddress.EncodeAddress(), nil
}
...
func BuildRuneEtchingCompleteRevealTx(privateKey *btcec.PrivateKey, runeOpReturnData []byte, runeCommitment []byte,
	...
	// 2. build reveal tx
	revealTx, _, revealFee, err := buildEmptyRevealTx(receiver, inscriptionScript, revealValue, feeRate, runeOpReturnData)
	if err != nil {
		return nil, 0, "", err
	}

	btcConnector := NewMempoolConnector()
	commitTx, err := btcConnector.GetRawTxByHash(commitTxHash)
	if err != nil {
		return nil, 0, "", err
	}
	revealTx, err = completeRevealTx(privateKey, commitTx, revealTx, inscriptionScript)
	...
	return revealTxBytes, revealFee, inscriptionAddress.EncodeAddress(), nil
}
...
func BuildRuneEtchingTxs(privateKey *btcec.PrivateKey, utxo []*Utxo, runeOpReturnData []byte, runeCommitment []byte,
	...
	inscriptionPkScript, _ := txscript.PayToAddrScript(inscriptionAddress)
	// 2. build reveal tx
	revealTx, totalPrevOutput, revealFee, err := buildEmptyRevealTx(receiver, inscriptionScript, revealValue, feeRate, runeOpReturnData)
	if err != nil {
		return nil, nil, 0, "", err
	}
	// 3. build commit tx
	...
	return commitTxBytes, revealTxBytes, revealFee + commitFee, inscriptionAddress.EncodeAddress(), nil
}
...
func BuildTransferBTCTx(privateKey *btcec.PrivateKey, utxo []*Utxo, toAddr string, toAmount, feeRate int64, net *chaincfg.Params, runeData []byte) ([]byte, error) {
	...
	// 1. build tx
	transferTx, _, err := buildCommitTx(utxo, wire.NewTxOut(toAmount, pkScript), feeRate, runeData, true)
	if err != nil {
		return nil, err
	}
	// 2.sign tx
...
func buildEmptyRevealTx(receiver btcutil.Address, inscriptionScript []byte, revealOutValue, feeRate int64, opReturnData []byte) (
	*wire.MsgTx, int64, int64, error) {
	...
	return tx, totalPrevOutput, fee, nil
}
...
func buildCommitTx(commitTxOutPointList []*Utxo, revealTxPrevOutput *wire.TxOut, commitFeeRate int64, runeData []byte, splitChangeOutput bool) (*wire.MsgTx, int64, error) {
	...
	if changeAmount > 0 {
		...
		if changeAmount < 0 {
			...
			if totalSenderAmount-btcutil.Amount(totalRevealPrevOutput)-feeWithoutChange < 0 {
				return nil, 0, errors.New(fmt.Sprintf("insufficient balance, total send:%d, reveal spent fee:%d, fee without change:%d", totalSenderAmount, btcutil.Amount(totalRevealPrevOutput), feeWithoutChange))
			}
		}
	}
	//clear mock witness
